...
class Post {
	...
	public function submitPost($body, $user_to) {
		$body = strip_tags($body); //Empêche l'injection d'HTML
		$body = mysqli_real_escape_string($this->con, $body);
		$check_empty = preg_replace('/\s+/', '', $body); //Enlève les espaces
		...
		if($check_empty != "") {
			...
			//Si la personne est sur son profil met aucun
			if($user_to == $added_by)
				$user_to = "aucun";
			...
			//Détermine si c'esst une url et dans ce cas ne met pas dans les trendings
	...
	public function loadProfilePosts($data, $limit) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			$num_iterations = 0; //Nombre de résultat retourné
			$count = 1;
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
					...
					//Une fois arrivé à 10 publications stop
					if($count > $limit) {
						break;
					}
					else {
				...
				<script>
					...
					$(document).ready(function() {
						...
						$('#post<?php echo $id; ?>').on('click', function() {
							bootbox.confirm("Êtes vous sûr de voulir supprimer votre publication ?", function(result) {
			...
			if($count > $limit)
				...
				$str .= "<input type='hidden' class='noMorePosts' value='true'><p style='text-align: centre;'> Fin des publications</p>";
		}
	...
	public function getSinglePost($post_id) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			$row = mysqli_fetch_array($data_query);
				...
				//Vérifie si le compte n'est pas fermé
				$added_by_obj = new User($this->con, $added_by);
				if($added_by_obj->isClosed()) {
					return;
				}
				...
				<?php
					...
					echo "<p>Vous ne pouvez pas voir ceci caar vous n'êtes pas ami</p>";
					return;
				}
		}
		else {
